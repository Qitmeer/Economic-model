...
在 Qitmeer 网络中，我们将面临着和比特币同样的难题，无法避免算力的集中化，但是如何让网络拥有庞大而分散的全节点网络是我们必须要思考的，这不仅是一个去中心化网络的基础，更是 Qitmeer 支撑普惠金融的需求与保障。因此，在 Qitmeer 系统设计中，矿工将不仅仅是区块奖励的唯一受益人，我们将为网络引入一个名为“星火节点”的角色。也就是说我们将对全节点进行经济激励，以建立起庞大而分散的全节点生态网络。

星火节点是一类运行着完整节点的特殊节点，他们将通过质押一定数量资产的方式向网络承诺实时在线服务并保证忠诚，同时，他们将获得来自区块奖励的回报。
星火节点的引入不仅能建立起庞大的全节点基数，更能促进网络生态的拓展，以及提高网络的易用性和用户的采用。所谓星星之火，可以燎原。Qitmeer对星火节点的激励，可以让开发者不用担心全节点的维护成本，甚至还可以通过星火节点获得回报，这对于在 Qitmeer 网络上建立商业应用是极为友好的。我们非常乐意见到 Qitmeer 的星星之火遍布大地，让 Qitmeer 网络的服务功能更加丰富。

未来，广大的星火节点将会演化成 Qitmeer 网络最重要的自治组织，他们是 Qitmeer 忠诚的守护者，将会是网络发展过程中最重要的影响者，乃至决策者。我们希望有更多的开发者能够采用 Qitmeer 网络来构建生态，我们也在考虑在 Qitmeer 网络建立生态财政预算，以鼓励和支持用户在 Qitmeer 网络上进行生态建设，而生态财政预算将对做出巨大贡献的节点给予支持和褒奖。
...
在 Qitmeer 的区块奖励计划设计中，我们将 50% 的区块奖励用于对矿工的激励，作为其提供硬件设备维护网络的回报；其次，将其余 50% 的区块奖励作为星火节点质押的收益或者其他激励。

在 Umayyad 时期，Qitmeer 主网平均出块时间设定为 30 秒，每个块的初始区块奖励设定为 20 Meer。按照这样的设定，Qitmeer 的每日初始发行量约为 57600 Meer，其中 50% 用于 PoW 矿工激励，50% 用于星火节点激励以及其他激励。由于 Qitmeer 的 MeerDAG 协议支持并发出块，因此每日的实际铸币量将大于 57600 Meer。


### 3.4 Qitmeer 星火节点

#### 3.4.1 为什么要成为 Qitmeer 的星火节点？

如果我们将 PoW 共识机制视为 Qitmeer 的第一层网络，那么 Qitmeer 的星火节点则好比是建立在第一层网络之上的第二层网络。第二层网络可以孵化出更多的服务和功能，丰富 Qitmeer 的生态场景，提高其金融服务能力。第一层网络验证着新产生的区块，为整个区块链提供了动力，让其能够平稳的向前，保证网络能够提供基本的功能和保障。第二层的星火节点网络则为整个区块链提供了额外的功能，从而提高了系统的易用性，可靠性。

对于星火节点为网络提供的服务，Qitmeer Network 为其设定了奖励，在 Qitmeer 的区块奖励中，将分出一部分用来奖励星火节点。Qitmeer 星火节点以 Meer 代币作为保证金，保证自己的忠诚，作为回报，星火节点持有人凭借星火节点提供的服务将定期获得奖励。因此，成为 Qitmeer 的星火节点，可以获得一定的经济收益，能够积累更多的代币资产，可以有效抵抗代币持有的通胀损失。


#### 3.4.2 如何成为 Qitmeer 的星火节点？

想要成为 Qitmeer 的星火节点，需要满足一定的条件：

1. 一台性能尚可的电脑或服务器，并配备公网 IP，当然，你不能经常关机

   由于星火节点需要为网络提供服务，这就需要经常保持在线，离线是不能获得收益的。此外，一定的硬件配置也是需要的，但这并不会太苛刻，普通家用电脑就可以胜任。为了保证 MainNode 能够时刻和网络保持连接，配备稳定的公网 IP 是很重要的。实际上，租用一台 VPS 服务器来运行星火节点是最简单的。

2. 一定数量的 Meer 代币

   想要运行一个星火节点，你必须持有指定数量的 Meer 代币作为你对 Qitmeer Network 的忠诚度证明，并且你需要将一定数量的代币存入一个独立的地址，在运行星火节点的时候这些代币将被锁定。

3. 简单的技术知识

   想要运行一个星火节点，你需要先准备一份配置文件，在配置文件里写入必要的参数。此外，你需要掌握一点 Linux 服务器相关的操作知识，但这也只是非常简单的计算机基础知识。


#### 3.4.3 Qitmeer 星火节点奖励计划

锁定一定数量的 Meer 保证金成为 Qitmeer 的星火节点，确保稳定在线为 Qitmeer Network 提供服务，将会定期获得区块奖励。

Qitmeer Network 每产生一个新的区块，都将有 50% 的区块奖励累积到星火节点奖励池，Qitmeer 的星火节点系统将根据 MainNodes 的有效在线时长定期发放奖励。

由于星火节点系统预计将在 Umayyad 网络上线一年后方可开发完成，因此更多细节将在后期版本中公布。
...
### 附件2：[Qitmeer_Umayyad_Network_星火节点体验计划](./Qitmeer_Umayyad_Network_主节点体验计划.md)
